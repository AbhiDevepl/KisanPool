...
export async function offersForRequest(requestId: string, farmerId: string) {
  ...
  // Optimization: Pre-fetch all referenced transporters and vehicles in batch
  // to avoid executing 2*N database queries inside the offer iteration loop.
  const transporterIds = Array.from(new Set(offers.map((o) => String(o.transporterId))));
  const vehicleIds = Array.from(new Set(offers.map((o) => String(o.vehicleId))));
