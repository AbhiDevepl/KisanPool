...
## ADR-042: The V2 targeted pass — Live Track hand-off, manual pickup, shared-machine utilisation
Date: 2026-08-29
Status: Accepted
Extends: ADR-030, ADR-035, ADR-038

Three additive changes, each keeping the existing V1/V2 workflow intact.

### A. Live Track hands off to Google Maps; it does not reimplement navigation
Context: The farmer's active-trip screen has an in-app map that moves the vehicle marker on every GPS tick, but no way to get turn-by-turn navigation to where the truck actually is. Rebuilding routing, ETAs, alternates and voice guidance in-app is neither cheap nor better than the maps app already on the phone.
Decision: `GET /pool/trips/:id/track` (trip-party only, reusing the authenticated trip identity — no public token) returns the transporter's latest known position (`Vehicle.currentLocation`, updated by the existing `vehicle:location` socket every ~5s), the trip's real destination mandi, and a ready `https://www.google.com/maps/dir/?api=1&origin=…&destination=…` deep link. A single `trackable` flag is driven by BUSINESS STATE, never a timer: false while the trip is FORMING, false once this viewer's own load is DELIVERED or later, false when the trip is COMPLETED/CANCELLED. `staleMinutes` is advisory only — an old position on a live trip is still the best the farmer has, so it is labelled, not withheld. Pooled trips share one stream: every authorised farmer calls the same endpoint and gets the same origin and destination. The farmer screen refreshes it on `trip:location` and `shipment:state`; the in-app map is unchanged.
Rationale: The URL carries only coordinates — no JWT, no ids — and Google needs coordinates to route anyway. Making expiry a function of the trip's state rather than a TTL means "delivered" genuinely ends exposure, and a slow trip never loses its link mid-journey. No Firebase, no second realtime backend: the position was already flowing over Socket.IO. Verified in `tests/10_v2_features.py`: forming → not trackable, EN_ROUTE → a directions URL to the mandi, a fresh GPS ping becomes the origin, a stranger gets AUTH_FORBIDDEN, two pooled farmers get one stream, and delivering a load expires that farmer's tracking by state.

### B. A pickup is the farmer's stated choice, not the device's GPS
Context: `requests/new.tsx` and `services/[id].tsx` both silently used `user.defaultLocation` — itself GPS-only, captured once at onboarding with no editor — as the pickup/field, with no UI to change it. A farmer standing in one village could not send produce, or call a machine, to another. This is the farmer the whole product is for: the one whose GPS is unreliable.
Decision: A reusable `<LocationPicker>` (GPS suggestion + `GET /maps/places` search + tap/drag the map pin + an editable name) is the single way pickup/field/base location is chosen, wired into request creation, machine booking, the farmer-details onboarding step and the new provider onboarding step. GPS is only ever a *suggestion* for the initial value; a manual choice is never overwritten, and GPS being denied or unavailable never blocks manual entry. `GET /maps/places` proxies Google Geocoding (key stays server-side) and degrades to an offline gazetteer of the demo corridor (`maps/places.ts`) so search is never dead. The submitted value is whatever the farmer confirmed.
Rationale: The fix had to be a place to enter the location, not a cosmetic label over a forced value — the same reasoning as ADR-037's missing-name fix. Everything downstream already keys on `request.pickup` / `booking.location`, so no matching or pricing change was needed. Verified end to end in `tests/10_v2_features.py`: a typed "Niphad, Nashik" is stored exactly, and the trip is then priced at the ~20 km Niphad→Lasalgaon route, not the ~220 km default.

### C. Shared-machine utilisation: co-scheduled nearby jobs split the travel, and only the travel
Context: ADR-038 built machinery hire as one provider, one farmer, one frozen quote — "there is nobody to split a cost with". The brief's core machinery ask introduces exactly that someone: when several farmers near each other want the same machine the same day, the provider serves them in one outing instead of one round trip per job, and the per-farmer cost should fall.
Decision: A deterministic, explainable grouping layer (`machinery/grouping.ts`), NOT a VRP optimiser — it groups compatible jobs on ONE machine, it does not sequence a fleet. `assessGrouping` scores a prospective hire against not-yet-started bookings on the same machine within 15 km and a working day (HIGH within ~8 km and the same part of the day), and reports the projected travel saving with the reason. `quoteBooking` gains `travelShareCount` (default 1, unchanged): when a job is grouped, the round-trip travel term divides by the group size — and ONLY travel, because an acre is an acre and an hour is an hour however many farmers share the drive out. A strongly compatible new REQUESTED booking auto-joins the cluster and re-quotes every not-yet-started member (the request-time analogue of ADR-035's pooled reallocation, bounded so a started job keeps the bill it began under); a provider can also group compatible requests explicitly via `POST /farm/bookings/group`. Each farmer sees their own reduced total; the provider sees the combined workload and earnings. Grouping is refused when the jobs are not actually near/soon — it is never forced to demo the feature.
Rationale: Splitting travel by "one round trip ÷ jobs on it" is grounded in what physically changes, so the discount is real, not an invented multiplier — the same discipline as ADR-035 and ADR-039. Bounding re-quotes to not-yet-started bookings preserves ADR-038's frozen-at-work guarantee. Verified in `tests/10_v2_features.py`: three nearby harvester jobs on consecutive slots auto-group, each farmer's travel drops to a third with work cost unchanged, farmer totals and the provider's combined total reconcile, and a distant job is assessed NONE.

### D. A machinery/service provider has an onboarding path
Context: Farmer and Transporter each have a role card and an onboarding flow; a machinery provider had neither — the only way to list a machine was a screen buried in Farm Services.
Decision: A third card on role selection, "Machinery / service provider", creates a FARMER account (ADR-038 — a provider is anyone who owns a machine, not a new role; the seed's providers are all FARMER, including the hiring centre) and routes to a new `(auth)/machine-register.tsx` step (name + one machine: category, title, operator mode, base location via the picker, service radius, pricing unit and rate) instead of the default-pickup step. They land in the farmer app, where Farm Services already carries the full provider surface (list more machines, the incoming-requests tab).
Rationale: Reusing the FARMER identity keeps ADR-002's permanent single role intact and avoids a role gate that would have excluded the farmer-owners the feature exists to unlock. No new stack, no routing rewrite.
