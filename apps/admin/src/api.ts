...
export const api = {
  ...
  /** Switch Redis OFF at runtime — failure-matrix CASE 3. Reversible. */
  disableRedis: () =>
    request<{ redis: { state: string; detail: string }; journal: ResilienceStatusDTO['journal'] }>(
      '/admin/resilience/redis/disable',
      { method: 'POST' },
    ),

  /** Connect/reconnect Redis. `url` overrides REDIS_URL without a restart. */
  enableRedis: (url?: string) =>
    request<{
      redis: { state: string; detail: string };
      journal: ResilienceStatusDTO['journal'];
      trustedForIntent: boolean;
      note: string;
    }>('/admin/resilience/redis/enable', {
      method: 'POST',
