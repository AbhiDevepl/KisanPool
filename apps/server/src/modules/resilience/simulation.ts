...
export function installGate(): void {
  ...
  mongoose.plugin((schema: mongoose.Schema) => {
    ...
    schema.pre('aggregate', function (next: (err?: Error) => void) {
      /*
       * `this.model()` must be called AS A METHOD.
       *
       * Pulling it off the Aggregate first (`const m = this.model; m()`) loses the
       * `this` binding, and Mongoose's accessor then reads `_model` off undefined
       * — which threw on every aggregate in the app, including the transporter's
       * own offers list. Keep the receiver.
       */
      const aggregate = this as unknown as {
        model?: () => { collection?: { collectionName?: string } } | undefined;
      };
      let collection = '';
      try {
        collection = aggregate.model?.()?.collection?.collectionName ?? '';
      } catch {
        // an un-modelled aggregate has no collection to gate on; let it through
      }
      next(gateFor(collection, false) ?? undefined);
    });
