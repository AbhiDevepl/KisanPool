...
export async function claimRequest(
  ...
  const intent = await recordIntent({
    ...
    operationKey: operationKey('OFFER_CLAIMED', requestId, transporterId),
    payload: { requestId, message: message ?? null },
  });
...
export async function withdrawOffer(offerId: string, transporterId: string) {
  ...
  const intent = await recordIntent({
    ...
    entityType: 'TransporterOffer',
    entityId: offerId,
    actorId: transporterId,
    operationKey: operationKey('OFFER_WITHDRAWN', offerId),
    payload: { requestId: String(offer.requestId) },
  });
